...
@implementation WRPayDueView

- (id) initWithPayDueNo:(NSString*)pdNoString{
    
    self = [super init];
    CGFloat cornerRadius = 12.f;
    CGFloat contentSize = 150.f;      //heigh
    self.backgroundColor = [UIColor clearColor];
    self.layer.cornerRadius = cornerRadius;
    self.pdNoString = pdNoString;
...
//    _titleLabel.layer.cornerRadius = cornerRadius;
    ...
    _contentText.frame = CGRectMake(0, 0, CGRectGetWidth(_contentView.frame), CGRectGetHeight(_contentView.frame));
    _contentText.backgroundColor = [UIColor WR_USC_Yellow];
    _contentText.textColor = [UIColor WR_USC_Red];
    _contentText.font = [UIFont boldSystemFontOfSize:18];
    _contentText.text = @"Hello\nWorld";
    _contentText.editable = NO;
    [_contentView addSubview:_contentText];
